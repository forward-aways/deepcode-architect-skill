...
- **全平台兼容**：原生支持 Cursor / Windsurf / GitHub Copilot 等主流 AI IDE
...
### Cursor / Windsurf
将 `system-prompt.md` 的完整内容复制到项目根目录的 `.cursorrules` 或 `.windsurfrules` 文件中：
```bash
cp system-prompt.md .cursorrules
```

### GitHub Copilot
Copilot 不支持 YAML Front Matter，需提取纯 Markdown 正文部分放入 `.github/copilot-instructions.md`：
```bash
...
- **Cross-Platform Compatible**: Natively supports Cursor, Windsurf, GitHub Copilot, and other mainstream AI IDEs
...
### Cursor / Windsurf
Copy the full content of `system-prompt.md` to `.cursorrules` or `.windsurfrules` in your project root:
```bash
cp system-prompt.md .cursorrules
```

### GitHub Copilot
Copilot does not support YAML Front Matter. Extract the pure Markdown body into `.github/copilot-instructions.md`:
```bash
sed '1,/^---$/d; /^---$/,$d' system-prompt.md > .github/copilot-instructions.md
```
